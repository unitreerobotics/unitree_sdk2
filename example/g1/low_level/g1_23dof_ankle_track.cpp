  ...
  void Control() {
    ...
    if (ms) {
      ...
      if (time_ < duration_) {
        ...
        float L_P_m = ms->q.at(LeftAnklePitch);
        float L_R_m = ms->q.at(LeftAnkleRoll);
        float R_P_m = ms->q.at(RightAnklePitch);
        float R_R_m = ms->q.at(RightAnkleRoll);
        // clang-format off
